...
const ACH_META={
  firstClear:{label:"첫 문제 클리어",grade:"일반"},streak3:{label:"3연속 성공",grade:"일반"},streak5:{label:"5연속 성공",grade:"일반"},streak10:{label:"10연속 성공",grade:"희귀"},streak20:{label:"20연속 성공",grade:"희귀"},streak30:{label:"30연속 성공",grade:"희귀"},streak50:{label:"50연속 성공",grade:"영웅"},streak100:{label:"100연속 성공",grade:"영웅"},clear150:{label:"150문제 클리어",grade:"전설"},
  fail1:{label:"첫 실패",grade:"일반"},failStreak2:{label:"2연속 실패",grade:"일반"},failStreak3:{label:"3연속 실패",grade:"일반"},failStreak5:{label:"5연속 실패",grade:"희귀"},failStreak10:{label:"10연속 실패",grade:"영웅"},
  speed5:{label:"5초 안에 성공",grade:"희귀"},speed3:{label:"3초 안에 성공",grade:"희귀"},speed2:{label:"2초 안에 성공",grade:"영웅"},speed1:{label:"1초 안에 성공",grade:"전설"},
  noHintSuccess:{label:"힌트 없이 성공",grade:"일반"},noHint10:{label:"힌트 없이 10문제 연속",grade:"희귀"},hintSuccess:{label:"힌트 보고 성공",grade:"일반"},hintFail:{label:"힌트 보고도 실패",grade:"일반"},
  nearOne:{label:"한 글자 차이",grade:"일반"},waitSuccess:{label:"10분 대기 후 성공",grade:"일반"},revengeSuccess:{label:"실패 후 바로 성공",grade:"희귀"},firstTryStreak5:{label:"5문제 연속 첫 시도 성공",grade:"희귀"},firstTryStreak10:{label:"10문제 연속 첫 시도 성공",grade:"영웅"},
  firstPuzzle8:{label:"첫 문제가 8글자",grade:"일반"},lastPuzzle2:{label:"마지막 문제가 2글자",grade:"일반"},firstVisit:{label:"첫 접속",grade:"일반"},visit100:{label:"100번째 접속",grade:"희귀"},
  lateNight:{label:"새벽 2~5시 플레이",grade:"일반"},commute:{label:"출근시간 8~9시",grade:"일반"},lunch:{label:"점심시간",grade:"일반"},rainy:{label:"비 오는 날/장마 단어",grade:"일반"},christmas:{label:"크리스마스 접속",grade:"희귀"},birthday:{label:"생일 접속",grade:"희귀"},
  allNoHint:{label:"전 문제 힌트 없이 클리어",grade:"전설"},allFirstTry:{label:"150문제 전부 첫 시도 성공",grade:"전설"},perfect150:{label:"150문제 100% 성공",grade:"전설"}
};
...
function renderAchievementList(){
  ...
  box.innerHTML=entries.length ? entries.map(({key,label,grade,msg})=>`
    <div class="puzzle-item ach-list-item">
      <b>${esc(label)}</b>
      <div><b>${esc(msg)}</b><div class="puzzle-meta">${esc(grade)} · ${esc(key)}</div></div>
...
function checkTimeAchievements(d, a){
  ...
  const m=new Date().getMonth()+1, day=new Date().getDate();
...
function achievementStats(d){
  const keys=d.achKeys||[];
  const grades=["일반","희귀","영웅","전설"];
  const out={전체:{got:keys.filter(k=>ACH[k]).length,total:Object.keys(ACH_META).filter(k=>ACH[k]).length}};
  grades.forEach(g=>{
    const total=Object.keys(ACH_META).filter(k=>ACH[k]&&ACH_META[k].grade===g).length;
    const got=keys.filter(k=>ACH[k]&&ACH_META[k]?.grade===g).length;
    out[g]={got,total};
  });
  return out;
}
function submit(){
  ...
  if(ok){
    round.gameOver=true;
    d.success++;
    d.totalTries+=round.guesses.length;
    d.streak++;
    d.failStreak=0;
    ...
    d.lastFailed=false;
    saveData(d);
    $("msg").textContent=`${round.guesses.length}/${CONFIG.maxTries} 성공 🎉\n${CONFIG.winMessages[round.guesses.length-1]}`;
    showAch(a.filter(Boolean));endButtons(true);
  }
    ...
    d.firstTryStreak=0;
    d.noHintStreak=0;
    d.failStreak=(d.failStreak||0)+1;
    d.lockUntil=Date.now()+CONFIG.lockMinutes*60*1000;
    d.wasLocked=true;
    d.lastFailed=true;
    d.allSuccess=false;
    d.allNoHint=false;
...
획득 업적
${(d.achKeys||[]).length?(d.achKeys||[]).map(k=>"・["+ACH_META[k]?.grade+"] "+(ACH_META[k]?.label||k)+" - "+(ACH[k]||"")).join("\n"):"없음"}`;
